...
/**
 * A {@link AlzzzzPlayer.RendererBuilder} for HLS.
 */
public class HlsRendererBuilder implements AlzzzzPlayer.RendererBuilder {
  ...
  @Override
  public void buildRenderers(AlzzzzPlayer player) {
  ...
  private static final class AsyncRendererBuilder implements ManifestCallback<HlsPlaylist> {
    ...
    private final Context context;
    private final String userAgent;
    private final AlzzzzPlayer player;
    private final ManifestFetcher<HlsPlaylist> playlistFetcher;

    private boolean canceled;

    public AsyncRendererBuilder(Context context, String userAgent, String url, AlzzzzPlayer player) {
      this.context = context;
      this.userAgent = userAgent;
      this.player = player;
    ...
    public void onSingleManifest(HlsPlaylist manifest) {
      ...
      HlsSampleSource sampleSource = new HlsSampleSource(chunkSource, loadControl,
          MAIN_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, mainHandler, player, AlzzzzPlayer.TYPE_VIDEO);
      ...
      if (haveSubtitles) {
        ...
        HlsSampleSource textSampleSource = new HlsSampleSource(textChunkSource, loadControl,
            TEXT_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, mainHandler, player, AlzzzzPlayer.TYPE_TEXT);
        textRenderer = new TextTrackRenderer(textSampleSource, player, mainHandler.getLooper());
      } else {
        textRenderer = new Eia608TrackRenderer(sampleSource, player, mainHandler.getLooper());
      }

      TrackRenderer[] renderers = new TrackRenderer[AlzzzzPlayer.RENDERER_COUNT];
      renderers[AlzzzzPlayer.TYPE_VIDEO] = videoRenderer;
      renderers[AlzzzzPlayer.TYPE_AUDIO] = audioRenderer;
      renderers[AlzzzzPlayer.TYPE_METADATA] = id3Renderer;
      renderers[AlzzzzPlayer.TYPE_TEXT] = textRenderer;
